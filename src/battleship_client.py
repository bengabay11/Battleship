...
def score(screen):
    ...
    img = pygame.image.load('../images/base.jpg')
    screen.blit(img, (0, 0))
    pygame.display.flip()
    finish = False
    text1 = pygame.font.SysFont("mono_space", 30)
...
def placed_ship(width, height):
    ...
    if (width, height) not in POSITIONS_OF_SHIPS and (width-30, height-30) not in POSITIONS_OF_SHIPS \
            and (width-30, height) not in POSITIONS_OF_SHIPS and (width, height-30) not in\
            POSITIONS_OF_SHIPS and (width+30, height + 30) not in POSITIONS_OF_SHIPS and (width+30, height)\
            not in POSITIONS_OF_SHIPS and (width, height+30) not in POSITIONS_OF_SHIPS and (width-30, height+30)\
            not in POSITIONS_OF_SHIPS and (width+30, height-30) not in POSITIONS_OF_SHIPS:
        return True
    return False
...
def create_board(screen):
    ...
    global POSITIONS_OF_SHIPS, FINISH_PUT_SHIPS, SEND_POSITIONS
    img = pygame.image.load('../images/base.jpg')
    screen.blit(img, (0, 0))
    for i in xrange(11):
    ...
    text1 = pygame.font.SysFont("mono_space", 30)
    text2 = pygame.font.SysFont("mono_space", 50)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
                ...
                if 170 < height < 470 and 30 < width < 330 and counter > 0:
                    ...
                    if placed_ship(width, height):
                        ...
                        ship1 = shapes.Ship(width, height)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
                    ...
                    if 170 < height < 470 and 370 < width < 670:
                        ...
                        while HIT == '':
                            pass
                        if HIT == 'success':
                            img = pygame.image.load('../images/vi.jpg')
                            ...
                            img = pygame.image.load('../images/X.jpg')
                            screen.blit(img, (width, height))
                            pygame.display.flip()
                            TURN = 'second'
                            HIT = ''
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
                    ...
                    if 16 < height < 73 and 21 < width < 77:
                        ...
                        break
            if HIT == 'success':
                img = pygame.image.load('../images/bom.jpg')
...
def main_game():
    ...
    img = pygame.image.load(r'../images/main.jpg')
    screen.blit(img, (0, 0))
    pygame.display.flip()
    mouse_pos_list = []
    pygame.display.flip()
...
def connect_to_server():
    ...
    global FINISH_PUT_SHIPS, HIT, TURN, RIVAL_POSITION, USERNAME2, SEND_POSITION, FINISH_CURRENT_GAME, SEND_POSITIONS,\
        BEGIN_GAME
    ...
    while FINISH_GAME is False:
        r_list, w_list, x_list = select.select([client_socket], [client_socket], [])
